...
interface Props {
  ...
  timeZone?: string;
  /** Hide the date control; combine time with `fixedDate` for the submitted value */
  hideDate?: boolean;
  /** Calendar day used when `hideDate` is true */
  fixedDate?: Date;
  /** Called whenever the combined local datetime value changes */
...
export function DateTimePicker({
  ...
  const [date, setDate] = useState<Date | undefined>(
    hideDate ? fixedDate : initial.date
  );
  ...
  const effectiveDate = hideDate ? fixedDate : date;

  useEffect(() => {
    if (!hideDate) return;
    setDate((prev) => (sameCalendarDay(prev, fixedDate) ? prev : fixedDate));
    setDateOpen(false);
  }, [hideDate, fixedDate]);

  const combined = combineValue(effectiveDate, time);
  ...
  useEffect(() => {
    ...
    if (required && !combined) {
      el.setCustomValidity(
        hideDate ? "Please select a time" : "Please select a date and time"
      );
    } else if (!required && !hideDate && effectiveDate && !time) {
      el.setCustomValidity("Please select a time, or clear the date");
    } else if (!required && hideDate && !effectiveDate && time) {
      el.setCustomValidity("Please select a start date first");
    } else {
      el.setCustomValidity("");
    }
  }, [required, combined, effectiveDate, time, hideDate]);
  ...
  return (
    <div ref={rootRef} className="space-y-1">
      ...
      <div className="flex flex-wrap gap-3">
        {!hideDate && (
          <div className="relative min-w-[10rem] flex-1">
            ...
            <button
              ...
              className="kawaii-input flex w-full cursor-pointer items-center justify-between gap-2 text-left"
            >
        ...
        <div className={`relative w-40 ${hideDate ? "" : "shrink-0"}`}>
          ...
          {timeOpen && (
            <div
              className={`absolute z-30 mt-2 flex gap-1 border border-border bg-surface p-2 shadow-lg ${
                hideDate ? "left-0" : "right-0"
              }`}
            >
              <TimeColumn
                label="Hour"
              ...
              {optional && time && (
                <div className="flex flex-col justify-end">
                  <button
                    type="button"
                    className="px-2 py-1.5 text-xs font-semibold text-ink-muted hover:text-ink"
                    onClick={() => {
                      setTime("");
                      setTimeOpen(false);
                    }}
